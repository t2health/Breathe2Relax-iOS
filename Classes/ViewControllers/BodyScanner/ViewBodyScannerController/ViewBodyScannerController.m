...
- (void)initDisplay	{
	...
	NSDictionary *ps = [NSPropertyListSerialization propertyListFromData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"bodyscanner" ofType:@"plist"]] 
	...
	[iv setImage:[UIImage imageNamed:bodyBundleName]];
    DEFAULTBODYASPECT = [appDelegate getImageAspect:iv.image];
    ...
    for (int i = 0; i < scannerCount ; i++ ) {
        NSString *imageName = [NSString stringWithFormat:@"%@%i.png",scannerBundleName,i];
        [maImg addObject:[UIImage imageNamed:imageName]];
    }
	...
	[iv setImage:[UIImage imageNamed:[scannerMask objectForKey:@"bundleName"]]];
	[scroller addSubview:iv];
	ivScanLineMask = iv;
	[iv release];
    ...
    for (NSDictionary *eachPart in parts ) {
        iv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 480, 825)];
        [iv setImage:[UIImage imageNamed:[eachPart objectForKey:@"bundleName"]]];
        iv.alpha = 0.0f;
...
- (void)updateLayout:(CGRect)frame {
	...
	h = w * [appDelegate getImageAspect:[UIImage imageNamed:@"ibreathe_body_scan0.png"]];
    ...
    for (int partCount = 0; partCount < [ivParts count]; partCount++) {
        ...
        if ([[thisPart objectForKey:@"overBody"] boolValue]) {
            ...
            w = partX * aspectRatio;
            h = w * [appDelegate getImageAspect:iv.image];
        }
        x = newSize.width/2.0 - w * 0.5f;
...
- (void)loadButtons{
	...
	UIImageView *iv;
	NSString *iconName;
	NSString *innerGlowName = @"bglow2.png";
	NSString *outerGlowName = @"bouterglow.png";
	CGFloat bx, by;
    ...
    for (partCount = 0; partCount < [parts count]; partCount++) {
		...
		[iv setImage:[UIImage imageNamed:outerGlowName]];
		iv.alpha = 0.0f;
		...
		[iv setImage:[UIImage imageNamed:innerGlowName]];
		iv.alpha = 0.0f;
		...
		[b setBackgroundImage:[UIImage imageNamed:iconName]  forState:UIControlStateNormal];
